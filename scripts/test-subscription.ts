...
async function main() {
	...
	if (room) {
		...
		console.log(
			"  Player UIDs in room:",
			Object.keys(data.players || {}).join(", "),
		);
		console.log("\n  Your UID:", userCred.user.uid);
		console.log(
			"  Are you a player?:",
			userCred.user.uid in (data.players || {}),
		);
	} else {
	...
	const unsubscribe = onSnapshot(
		...
		(snapshot) => {
			...
			if (snapshot.exists()) {
				...
				const flipped = (data.cards || []).filter(
					(c: any) => c.isFlipped && !c.isMatched,
				);
				console.log(
					"  flipped (unmatched):",
					flipped.map((c: any) => c.id).join(", ") || "none",
				);
			} else {
	...
	console.log(
		"(Note: You won't receive updates if you're not a player in the room due to security rules)",
	);
