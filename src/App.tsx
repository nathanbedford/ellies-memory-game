...
function App() {
		...
		async (options: {
			...
			try {
				...
				const currentGameRound =
					(onlineGame.gameState as OnlineGameState)?.gameRound || 0;
			...
			if (isOnlineMode) {
				...
				try {
					...
					const currentVersion =
						(reconciledState as OnlineGameState).syncVersion || 0;
					...
					await adapter.setState(onlineState);
					console.log("[RECONCILE] Synced reconciled state to Firestore", {
						version: newVersion,
					});
				} catch (error) {
	...
	}, [gameState.gameStatus, gameState.cards.length, isMobileScreen]);
	...
	}, [autoSizeEnabled, isMobileScreen]);
	...
	}, [autoSizeEnabled, gameState.cards.length, isMobileScreen]);
	...
	useEffect(() => {
		if (gameState.gameStatus === "playing" && gameState.cards.length > 0) {
			...
			if (totalScore === expectedPairCount) {
				console.log(
					`[RECONCILE] Total score reached ${expectedPairCount} (game complete), reconciling matched cards`,
				);
				reconcileScores();
			}
		}
		...
		<>
			...
			<div className={`min-h-screen ${isPlaying ? "pt-4" : "py-8"} ${!isStandalonePage ? "overflow-hidden" : ""}`}>
				<div className="container mx-auto px-4 max-w-full">
								...
								>
									...
									>
										<GameBoard
											...
											onCursorMove={
												cursorSyncEnabled ? handleCursorMove : undefined
											}
											onCursorLeave={
					...
					<Modal
						isOpen={
							setupStep === "cardPack" && currentPath === "/local/card-pack"
						}
						onClose={cancelSetupFlow}
						title={
							isResetting
					...
					<Modal
						isOpen={
							setupStep === "cardBack" && currentPath === "/local/card-back"
						}
						onClose={cancelSetupFlow}
