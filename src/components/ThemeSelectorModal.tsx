...
export const ThemeSelectorModal = ({ onSelectTheme, onBuildCustom }: ThemeSelectorModalProps) => {
  const [selectedCategory, setSelectedCategory] = useState<'all' | GameTheme['category']>('all');

  const handleThemeSelect = (theme: GameTheme) => {
    logWizardInteraction('Theme selected', { themeId: theme.id, themeName: theme.name });
    onSelectTheme(theme);
  };
  ...
  const categories: Array<{ id: 'all' | GameTheme['category']; label: string; emoji: string }> = [
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-xl font-bold text-gray-800 mb-2">Choose Your Theme</h3>
        <p className="text-gray-600 text-sm">Pick a pre-built theme or build your own custom setup</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="text-lg font-bold text-gray-800 mb-1">{theme.name}</h4>
        ...
        >
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <h4 className="text-lg font-bold text-gray-800 mb-2">Build Your Own</h4>
