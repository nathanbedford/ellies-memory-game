...
export const ScoreBoard = ({ players, cards, currentPlayer }: ScoreBoardProps) => {
  return (
      ...
      <div className="flex justify-around">
        {players.map((player) => (
          ...
          >
            ...
            <p className="text-3xl font-bold text-gray-900 mt-2">{getPlayerScore(cards, player.id)}</p>
            {currentPlayer === player.id && (
              <p className="text-sm text-blue-600 mt-2 font-medium">Current Turn</p>
            )}
          </div>
        ))}
      </div>
