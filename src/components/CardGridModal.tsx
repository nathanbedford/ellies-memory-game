...
interface CardGridModalProps {
    ...
    renderCard?: (card: Card, index: number, onCardClick: (index: number) => void) => ReactNode; // Optional custom card renderer
}
...
export const CardGridModal = ({
    ...
    const defaultRenderCard = (card: Card, index: number, onCardClick: (index: number) => void) => (
        <div
            key={card.id}
                    ...
                    <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
                        {cards.length === 0 ? (
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
                                {cards.map((card, index) => cardRenderer(card, index, handleCardClick))}
                            </div>
                        )}
                    </div>
                </div>
