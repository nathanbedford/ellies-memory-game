...
export const PairCountModal = ({ selectedPairCount, onSelect }: PairCountModalProps) => {
  ...
  const getDifficulty = (count: number) => {
    if (count <= 6) return { label: 'Easy', color: 'text-green-600', bg: 'bg-green-50' };
    if (count <= 12) return { label: 'Medium', color: 'text-yellow-600', bg: 'bg-yellow-50' };
        ...
        {PAIR_COUNT_OPTIONS.map((count) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-xs mt-1 px-2 py-0.5 rounded-full font-medium ${difficulty.color} ${difficulty.bg}`}>
                {difficulty.label}
              </span>
            </button>
          );
