					...
					{isTie ? (
						<div className="mb-6">
							...
							<p className="text-lg text-gray-500">
								{players.find((p) => p.id === 1)?.name || "Player 1"} and{" "}
								{players.find((p) => p.id === 2)?.name || "Player 2"} both
								scored {getPlayerScore(cards, 1)} pairs!
							</p>
						</div>
					) : (
						<p className="text-xl text-gray-600 mb-6">
							{winner?.name} wins with{" "}
							{winner ? getPlayerScore(cards, winner.id) : 0} pairs!
						</p>
					)}
