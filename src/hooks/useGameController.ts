...
export function useGameController(options: UseGameControllerOptions): GameControllerReturn {
  const {
    mode,
    initialGameState,
    initialSettings,
  ...
  const isAuthoritative = !isOnlineMode || localPlayerSlot === gameState.currentPlayer;
  ...
  const checkForMatch = useCallback((_selectedIds: string[], currentState: GameState) => {
    ...
    const currentPlayerName = getPlayerById(currentState.players, currentPlayerId)?.name || `Player ${currentPlayerId}`;
    ...
    if (isMatch) {
      ...
      setTimeout(() => {
        setGameState(prevState => {
          ...
          if (finalState.gameStatus === 'finished') {
            effectManager?.notifyGameOver(finalState.winner, finalState.isTie);
          }
      ...
      const nextPlayerName = getPlayerById(noMatchState.players, nextPlayerId)?.name || `Player ${nextPlayerId}`;
  ...
  }, [gameState, isOnlineMode, localPlayerSlot, settings.flipDuration, syncToFirestore, checkForMatch]);
  ...
  const endTurn = useCallback(() => {
    ...
    const nextPlayerName = getPlayerById(newState.players, nextPlayerId)?.name || `Player ${nextPlayerId}`;
    effectManager?.notifyTurnChange(nextPlayerName, nextPlayerId);
  ...
  const startGame = useCallback(() => {
    ...
    const firstPlayerName = getPlayerById(gameState.players, firstPlayerId)?.name || `Player ${firstPlayerId}`;
  ...
  const updateSettings = useCallback((newSettings: Partial<GameSettings>) => {
    setSettings(prev => {
      ...
      if ('autoSizeEnabled' in newSettings) {
        localStorage.setItem('autoSizeEnabled', String(updated.autoSizeEnabled));
      }
      if ('useWhiteCardBackground' in newSettings) {
        localStorage.setItem('useWhiteCardBackground', String(updated.useWhiteCardBackground));
      }
      if ('emojiSizePercentage' in newSettings) {
        localStorage.setItem('emojiSizePercentage', String(updated.emojiSizePercentage));
      }
  ...
  const updateLayoutMetrics = useCallback((metrics: LayoutMetrics) => {
    setLayoutMetrics(prev => {
      ...
      if (
        ...
        Math.round(prev.boardAvailableHeight) === Math.round(metrics.boardAvailableHeight) &&
        Math.round(prev.scoreboardHeight) === Math.round(metrics.scoreboardHeight)
      ) {
        return prev;
      }
      return metrics;
  ...
  const calculateOptimalCardSize = useCallback((cardCount: number): number => {
