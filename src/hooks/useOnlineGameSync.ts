...
export function useOnlineGameSync(options: UseOnlineGameSyncOptions) {
  ...
  const syncToFirestore = useCallback(async (state: GameState) => {
    if (!isOnline || !roomCode || localPlayerSlot === null) {
      console.log('[SYNC] Skipping sync - not ready', { isOnline, roomCode, localPlayerSlot });
      return;
    }
    ...
    try {
      ...
      const currentVersion = (state as OnlineGameState).syncVersion || lastSyncedVersionRef.current || 0;
